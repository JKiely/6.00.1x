...
class PhraseTrigger(Trigger):
    def __init__(self, phrase):
        self.phrase = phrase
    def evaluate(self, story):
        text = story.getSummary() + story.getTitle() + story.getSubject()
...
def filterStories(stories, triggerlist):
    ...
    for story in stories:
        for trigger in triggerlist:
            if trigger.evaluate(story) and not trigger in trstories:
                trstories.append(story)
    return trstories
...
def main_thread(master):
    ...
    try:
        ...
        triggerlist = readTriggerConfig("C:\Users\JKiely\Google Drive\Documents\\6.00x\\triggers.txt")
